...
export const CommitsGrid = ({
  ...
  const generateHighlightedCells = (text: string, fixedWidth?: number) => {
    const cleanedText = cleanString(text);

    const width = fixedWidth
      ? fixedWidth
      : Math.max(cleanedText.length * 6, 6) + 1;

    const textWidth = cleanedText.length * 6;
    const padding = fixedWidth ? fixedWidth - textWidth : 0;
    const offset = Math.floor(padding / 2);

    let currentPosition = 1 + offset;
  ...
  const {
    ...
    width: gridWidth,
    height: gridHeight,
  } = generateHighlightedCells(text, fixedWidth);

  const getRandomColor = () => {
    const commitColors = ["#4AE168", "#0AC740", "#02A232"];
  ...
  return (
    <section
      className={cn(
        "w-full bg-black  grid p-1 sm:p-3 gap-0.2 sm:gap-1 rounded-[5px] sm:rounded-[15px]",
        className,
      )}
      style={{
    ...
    >
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={`border border-white h-full w-full aspect-square rounded-[4px] sm:rounded-[3px] ${
